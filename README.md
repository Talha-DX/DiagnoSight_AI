**DiagnoSight AI 🏥
An AI-powered medical diagnostic assistant that helps doctors analyze medical images and symptoms with using Deep Learning using TensorFlow.**

**what we add:**

Authenication Properly
A ChatBot
A proper Report saving system
A Diagnosis Tab where Diseases identify with accuary of 90%+ 

**What Problems Industry facing**

Long wait times for diagnoses
Human error in medical image interpretation
Specialist shortages in underserved areas
Inconsistent diagnostic quality

**We Provvide Solutions**

Instant Analysis: Process X-rays, MRIs, CT scans in seconds
AI Accuracy: 99.2% accuracy across 15+ conditions
Symptom Correlation: Cross-reference images with patient symptoms
Clinical Support: Evidence-based recommendations for doctors

**Screen Shots**

### 1) Login Screen
![Login Screen](./assets/images/AI_1.png)

### 2) Main DashBoard Dashboard (without Login)
![Main DashBoard](./assets/images/AI_2.png)


### 3) Main DashBoard Dashboard (with Login)
![Main DashBoard](./assets/images/AI_3.png)


### 4) Login Page
![Login Page](./assets/images/AI_4.png)

### 5) Signup Page
![SignUp Page](./assets/images/AI_5.png)

### 6) Diagnsis Tab
![Diagnosis Tab](./assets/images/AI_6.png)

### 7) ChatBot Tab
![chatBot Tab](./assets/images/AI_7.png)

### 8) Doctor Recommendation Tab
![Doctor Tab](./assets/images/AI_8.png)

### 9) Report Tab
![Report Tab](./assets/images/AI_9.png)

### 10) PDF Download Format
![PDF Download](./assets/images/AI_10.png)

### 11)  Setting Tab
![Setting Tab](./assets/images/AI_11.png)

### 12) Edit Profile 
![Profile](./assets/images/AI_12.png)

### 13) Change Password
![Password](./assets/images/AI_13.png)

### 14) Logout Confirmation
![Logout](./assets/images/AI_14.png)

### 15) Delete Account
![Delete](./assets/images/AI_15.png)






